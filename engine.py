...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, 
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        out = model(image, sentences)
        loss, loss_masks, loss_contrastive = criterion(out, target) 
        ...
        # for evaluation
        with torch.no_grad():
            low_res_logits = out['pred_masks'][:, slice(0,1), :, :]
        ...
        del image, target, sentences, data, \
            low_res_logits, loss, loss_masks, loss_contrastive, \
            low_res_masks, iou, I, U, out
...
def eval_train(model, data_loader, epoch, criterion,
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 50, header):
            ...
            del image, target, sentences, data, \
                low_res_logits, low_res_masks, iou, I, U, idx, \
                loss, loss_masks, loss_contrastive, out 
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            for j in range(sentences.size(-1)):
                out = model(image, sentences[:, :, :, j])
                low_res_logits = out['pred_masks']
                low_res_masks = torch.where(low_res_logits > 0.0, 1, 0)
            ...
            del image, target, sentences, data, \
                ...
                low_res_logits, low_res_masks, low_res_mask, I, U, this_iou, out
